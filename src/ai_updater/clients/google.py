...
from ..ai_updater_utils import calculate_cost, read_file_content, write_to_file
from ..prompts.applychanges_prompts import (
...
from ..prompts.getrelevantcontext_prompts import (
    GETRELEVANTCONTEXT_P1,
    GETRELEVANTCONTEXT_P2,
    GETRELEVANTCONTEXT_S1,
    GETRELEVANTCONTEXT_S2,
)
...
class Google(Client):
    ...
    async def get_relevant_context(
        self, git_diff_output: str, sdk_tree_output: str, tests_tree_output: str
    ) -> list[ContextInclusion]:
        ...
        print(
            f"Finished get_relevant_context stage 1. Gemini model used: {response.model_version}"
        )
        self.total_cost += calculate_cost(
            response.usage_metadata, response.model_version
        )
        if self.args.debug:
            if self.args.work:
                ...
                write_to_file(
                    os.path.join(self.current_dir, "getrelevantcontext_stage1.txt"),
                    str(response.text),
                    quiet=True,
                )
        ...
        for file_path in response.parsed.file_paths:
            file_content = f"File path: {file_path}\n" + read_file_content(
                os.path.join(self.sdk_root_dir, file_path)
            )
            prompt = GETRELEVANTCONTEXT_P2.format(
                git_diff_output=git_diff_output, file_content=file_content
            )
        ...
        if self.args.debug:
            if self.args.work:
                ...
                write_to_file(
                    os.path.join(self.current_dir, "getrelevantcontext_stage2.txt"),
                    analysis_str,
                    quiet=True,
                )
        print(
            f"Finished get_relevant_context stage 2. Gemini model used: {file_analysis[0].model_version}"
        )
        return [response.parsed for response in file_analysis]

    def get_diff_analysis(
        self, git_diff_output: str, relevant_files: list[ContextInclusion]
        ...
        prompt = DIFFPARSER_P.format(
            git_diff_output=git_diff_output, selected_context_files=relevant_context
        )
        response = self.client.models.generate_content(
            model="gemini-2.5-flash",
            contents=prompt,
        ...
        if self.args.debug:
            if self.args.work:
                ...
                write_to_file(
                    os.path.join(self.current_dir, "getdiffanalysis.txt"),
                    response.text,
                    quiet=True,
                )
        print(
            f"Finished get_diff_analysis. Gemini model used: {response.model_version}"
        )
        return response

    def generate_pr_summary(
        self, git_diff_output: str, diff_analysis: types.GenerateContentResponse
    ):
        ...
        """
        prompt = GENERATESUMMARY_P.format(
            git_diff_output=git_diff_output, diff_analysis_text=diff_analysis.text
        )
        response = self.client.models.generate_content(
        ...
        write_to_file(
            os.path.join(self.current_dir, "pr_summary.txt"), response.text, quiet=True
        )
        print(
            f"Finished generating PR summary. Gemini model used: {response.model_version}"
        )

    def generate_patch(
        self, file_path: str, implementation_detail: str, ai_file_path: str
    ):
        ...
        """
        existing_file_content = read_file_content(
            os.path.join(self.sdk_root_dir, file_path)
        )
        initial_prompt_text = GENERATEPATCH_P.format(
        ...
        contents = [
            types.Content(role="user", parts=[types.Part(text=initial_prompt_text)])
        ]
        patch_success = False
        stop_trying = False
        attempt_count = 0
                ...
                if (
                    ...
                    if function_call.name == "apply_patch":
                        ...
                        if patch_success:
                            ...
                            final_replacement_text = function_call.args[
                                "replacement_text"
                            ]
                        ...
                        print(
                            f"Unexpected function call: {function_call.name}. Aborting patch attempts."
                        )
                        patch_success = False
                        stop_trying = True
                else:
                    print(
                        "No function call was made by the AI. Aborting patch attempts."
                    )
                    patch_success = False
                    stop_trying = True
            else:
                print(
                    "No response candidates or content found from AI. Aborting patch attempts."
                )
                patch_success = False
                stop_trying = True
        ...
        if patch_success:
            ...
            print(
                f"Failed to patch {file_path}. Falling back to complete file generation.\n"
            )
            self.generate_file(
                file_path=file_path,
                implementation_detail=implementation_detail,
    ...
    def generate_file(
        ...
        if fallback:
            ...
            prompt = GENERATECOMPLETEFILE_P.format(
                implementation_detail=implementation_detail,
                existing_file_content=f"==={file_path}===\n{existing_file_content}",
            )
        else:
    ...
    def apply_changes(self, diff_analysis: types.GenerateContentResponse):
        ...
        if len(parsed_response.files_to_update) != len(
            parsed_response.implementation_details
        ):
            raise ValueError(
                "ERROR: AI OUTPUT A DIFFERENT NUMBER OF FILENAMES THAN IMPLEMENTATION DETAILS"
            )
        if len(parsed_response.files_to_update) == 0:
            print(
                "THE AI WORKFLOW DID NOT DETERMINE THAT ANY FILES NEED TO BE UPDATED BASED ON THE GIVEN PROTO UPDATE DIFF"
            )
            return
        ...
        for i in range(len(parsed_response.files_to_update)):
            ...
            original_file_dir = os.path.dirname(
                os.path.join(self.sdk_root_dir, file_path)
            )
            original_filename = os.path.basename(file_path)
            if self.args.test:
                dir_structure = os.path.relpath(original_file_dir, self.sdk_root_dir)
                ai_generated_dir = os.path.join(
                    os.path.dirname(self.sdk_root_dir), "ai_generated", dir_structure
                )
        ...
        print("Finished applying changes. Gemini model used: gemini-2.5-flash")
    ...
    def configure_sdk_specifics(self, sdk: str) -> dict:
        ...
        if sdk == "python":
            ...
            git_diff_output = subprocess.check_output(
                ["git", "diff", "HEAD~1", "HEAD", "--", git_diff_dir, ":!*_pb2.py"],
                text=True,
                cwd=self.sdk_root_dir,
            )
            sdk_tree_output = subprocess.check_output(
                ["tree", os.path.join("src", "viam")], text=True, cwd=self.sdk_root_dir
            )
            tests_tree_output = subprocess.check_output(
                ["tree", "tests"], text=True, cwd=self.sdk_root_dir
            )
        elif sdk == "cpp":
            ...
            diff_exclude_files = [
                ...
                ":!src/viam/api/buf.yaml",
                ":!src/viam/api/CMakeLists.txt",
                ":!src/viam/api/viamcppsdk_replace_switch.cmake",
            ]
            git_diff_command = [
                "git",
                "diff",
            ...
            git_diff_output = subprocess.check_output(
                git_diff_command, text=True, cwd=self.sdk_root_dir
            )
            sdk_tree_output = subprocess.check_output(
                ["tree", os.path.join("src", "viam", "sdk")],
                text=True,
                cwd=self.sdk_root_dir,
            )
            ...
            git_diff_output = subprocess.check_output(
                ["git", "diff", "HEAD~1", "HEAD", "--", git_diff_dir],
                text=True,
                cwd=self.sdk_root_dir,
            )
            sdk_tree_output = subprocess.check_output(
                ["tree", os.path.join("lib", "src"), "-I", "gen"],
                text=True,
                cwd=self.sdk_root_dir,
            )
            tests_tree_output = subprocess.check_output(
                ["tree", "test"], text=True, cwd=self.sdk_root_dir
            )
        elif sdk == "typescript":
            current_commit = subprocess.check_output(
                ["git", "rev-parse", "HEAD"], text=True, cwd=self.sdk_root_dir
            ).strip()
            subprocess.check_output(
                ["git", "checkout", "HEAD~1"], cwd=self.sdk_root_dir
            )
            subprocess.check_output(["make", "build-buf"], cwd=self.sdk_root_dir)
            subprocess.check_output(
                ["mv", "src/gen", "../HEAD-1"], cwd=self.sdk_root_dir
            )
            subprocess.check_output(
                ["git", "checkout", current_commit], cwd=self.sdk_root_dir
            )
            subprocess.check_output(["make", "build-buf"], cwd=self.sdk_root_dir)
            subprocess.check_output(["mv", "src/gen", "../HEAD"], cwd=self.sdk_root_dir)
            git_diff_output = subprocess.run(
                ["diff", "-r", "-u", "--exclude='.*'", "../HEAD-1", "../HEAD"],
                text=True,
                capture_output=True,
                cwd=self.sdk_root_dir,
            ).stdout
            sdk_tree_output = subprocess.check_output(
                ["tree", "src"], text=True, cwd=self.sdk_root_dir
            )
            tests_tree_output = "\nFor the Typescript SDK, the tests are included within the src directory (as .spec.ts files)."
            subprocess.check_output(
                ["rm", "-rf", "../HEAD-1", "../HEAD"], cwd=self.sdk_root_dir
            )
        else:
            raise ValueError(
                f"Invalid SDK: {sdk}. The AI updater currently only supports python, cpp, typescript, and flutter."
            )
        return {
            "git_diff_output": git_diff_output,
            "sdk_tree_output": sdk_tree_output,
            "tests_tree_output": tests_tree_output,
        }
    ...
    async def run(
        ...
        if git_diff_output == "":
            print(
                "There were no proto changes detected that required an update to the SDK. Exiting."
            )
            write_to_file(
                os.path.join(self.current_dir, "pr_summary.txt"),
                "No changes were needed to the SDK.",
                quiet=True,
            )
            return
        if self.args.debug:
            if self.args.work:
                ...
                write_to_file(
                    os.path.join(self.current_dir, "gitdifftest.txt"),
                    git_diff_output,
                    quiet=True,
                )

        relevant_context = await self.get_relevant_context(
            git_diff_output, sdk_tree_output, tests_tree_output
        )
