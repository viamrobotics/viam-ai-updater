...
def apply_patch(
    ...
    if len(search_text) != len(replacement_text):
        ...
        if not quiet:
            print(
                f"ERROR: Mismatched list lengths - {len(search_text)} search blocks but {len(replacement_text)} replacement blocks"
            )
        return {
            "success": False,
    ...
    for i, (search, replace) in enumerate(zip(search_text, replacement_text)):
        ...
        if search_count == 0:
            ...
            if not quiet:
                print(
                    f"ERROR: Patch {i + 1}: Search text not found in file. The AI needs to generate a search block that exists in the file exactly as written."
                )
            return {
                "success": False,
            ...
            if not quiet:
                print(
                    f"ERROR: Patch {i + 1}: Search text appears {search_count} times in file. The AI must include more surrounding context to make the search block unique."
                )
            return {
                "success": False,
